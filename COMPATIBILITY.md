...
Qwik support is experimental and optimizer-only. Stable Qwik 1.20.0 excludes Vite 8, while Qwik 2 beta supports it. Qwik 2 beta.36 and beta.37 still fail semantic checking of their SVG JSX declarations under TypeScript 6 and TypeScript 7, and their runtime ESM reads optimizer globals during direct Node evaluation. Only `packages/qwik/tsconfig.json` enables `skipLibCheck`; adapter source, tests, generated declarations, optimizer output, and real-browser identity behavior remain checked. The peer is exact until Qwik 2 becomes stable and both upstream exceptions are removed.
