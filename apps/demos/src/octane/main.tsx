...
import { useLikftc } from "@vp-tw/likftc/octane";
...
function Panel({
  ...
  return (
    <article className="runtime-panel">
      <small>{corrected ? "WITH @LIKFTC/OCTANE" : "WITHOUT"}</small>
...
await mountFrameworkDemo("Octane", (target, initialState): DemoRuntime => {
  const root = createRoot(target);
  const update = (state: DemoFrameState): void => {
    flushSync(() => root.render(<Runtime state={state} />));
  };
  update(initialState);
  return {
